...
function GamePhotos({ game, onAdd, onRemove }) {
  ...
  const [lightbox, setLightbox] = useState(null); // index of photo to show full-screen
            ...
            <div className="absolute top-2 right-2 flex gap-2">
              {lightbox > 0 && (
                <button onClick={() => setLightbox(lightbox - 1)}
                  className="bg-black/50 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-black/70">‹</button>
              )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {rankings.map((r, i) => (
                    <tr key={r.name} className="border-b border-slate-100 odd:bg-white even:bg-slate-50/40">
          ...
          <div className="space-y-4">
            {sortedGames.length === 0 && (
              <div className="bg-white rounded-xl p-8 text-center text-slate-400 border border-slate-200">
                No games recorded for this month.
              </div>
            )}
            {sortedGames.map((game) => {
              const sorted = [...game.scores].sort((a, b) => b.score - a.score);
              return (
                <div key={game.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                    <span className="font-semibold text-slate-700">{formatDateLong(game.date)}</span>
                    {game.hostName && (
                      <span className="text-xs text-slate-400">Host: {game.hostName}</span>
                    )}
                  </div>
                  <div className="divide-y divide-slate-100">
                    {sorted.map((s) => (
                      <div key={s.playerId || s.playerName} className="flex items-center justify-between px-5 py-2.5">
                        <span className="text-sm text-slate-700">{s.playerName}</span>
                  ...
                  <GamePhotos
                    game={game}
                    onAdd={(b64) => addGamePhoto(game.id, b64)}
                    onRemove={(i) => removeGamePhoto(game.id, i)}
                  />
                </div>
              );
            })}
